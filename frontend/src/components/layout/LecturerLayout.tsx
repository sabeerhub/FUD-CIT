...
export default function LecturerLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { icon: Bell, label: 'Post Announcement', path: '/lecturer/announcements' },
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={cn(
                ...
                "hover:bg-emerald-600/10 hover:text-emerald-600 group"
              )}
            >
              <item.icon className="w-5 h-5 text-slate-500 group-hover:text-emerald-600" />
              <span className="font-medium text-slate-700 dark:text-slate-300 group-hover:text-emerald-600">
                {item.label}
              </span>
            </button>
          ))}
